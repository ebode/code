...
#include "D1SteppingMessenger.hh"
...
D1SteppingAction::D1SteppingAction(D1RecorderBase* r)
  : fRecorder(r),fOneStepPrimaries(false)
{
  fSteppingMessenger = new D1SteppingMessenger(this);
...
void D1SteppingAction::UserSteppingAction(const G4Step * theStep){

  G4Track* theTrack = theStep->GetTrack();

  if ( theTrack->GetCurrentStepNumber() == 1 ) fExpectedNextStatus = Undefined;
 
  D1UserTrackInformation* trackInformation
    =(D1UserTrackInformation*)theTrack->GetUserInformation();
    ...
    if(thePostPoint->GetStepStatus()==fGeomBoundary){
      if(fExpectedNextStatus==StepTooSmall){
        if(boundaryStatus!=StepTooSmall){
          ...
          ed << "D1SteppingAction::UserSteppingAction(): "
                << "No reallocation step after reflection!"
                << G4endl;
          G4Exception("D1SteppingAction::UserSteppingAction()", "D1Expl01",
          FatalException,ed,
     ...
     G4SDManager* SDman = G4SDManager::GetSDMpointer();
        G4String sdName="/Shape1SD";
        D1Shape1SD* Shape1SD = (D1Shape1SD*)SDman->FindSensitiveDetector(sdName);
        if(Shape1SD)Shape1SD->ProcessHits_constStep(theStep,NULL);
        trackInformation->AddTrackStatusFlag(hitShape1);

     
       

   //  G4SDManager* SDman = G4SDManager::GetSDMpointer();
       // G4String sdName="/Shape2SD";
        D1Shape2SD* Shape2SD = (D1Shape2SD*)SDman->FindSensitiveDetector(sdName);
        if(Shape2SD)Shape2SD->ProcessHits_constStep(theStep,NULL);
        trackInformation->AddTrackStatusFlag(hitShape2);       
    ...
    SDman->AddNewDetector(Shape1);
     ...
     G4SDManager* SDman = G4SDManager::GetSDMpointer();
     G4String sdName="Shape1";
       G4MultiFunctionalDetector* absDetector = (G4MultiFunctionalDetector*)SDman->FindSensitiveDetector(sdName);
       if(absDetector)absDetector->ProcessHits_constStep(theStep,NULL);
     */ 
      trackInformation->AddTrackStatusFlag(hitShape1);
       break;
      }
